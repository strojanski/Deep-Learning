...
INPUT_SIZE  = train_data.shape[0]   # 3072 for CIFAR-10
OUTPUT_SIZE = 10
...
def grid_search():
    ...
    for hidden, lr, bs in itertools.product(hidden_configs, learning_rates, batch_sizes):
        ...
        net.train(train_data, train_class, val_data, val_class,
                  epochs=epochs, mini_batch_size=bs, eta=lr)

        acc, loss = evaluate(net, val_data, val_class)
        config = {"hidden": hidden, "lr": lr, "batch_size": bs,
                  "val_acc": round(acc, 4), "val_loss": round(loss, 4)}
...
def regularisation(best_config):
    ...
    for optimizer, lambda_, label in experiments:
        ...
        net.train(train_data, train_class, val_data, val_class,
                  epochs=epochs, mini_batch_size=bs, eta=lr_map[optimizer])
        acc, loss = evaluate(net, test_data, test_class)
        print(f"  Test acc={acc:.4f}  Test loss={loss:.4f}")

# ─────────────────────────────────────────────
# Task 4 — Learning Rate Schedule
# ─────────────────────────────────────────────
def lr_schedule(best_config):
    ...
    for decay, decay_rate, label in experiments:
        print(f"\n  {label}")
        net = Network(sizes, optimizer="adam", lambda_=0.001,
                      decay=decay, decay_rate=decay_rate)
        net.train(train_data, train_class, val_data, val_class,
                  epochs=epochs, mini_batch_size=bs, eta=0.001)
        acc, loss = evaluate(net, test_data, test_class)
        print(f"  Test acc={acc:.4f}  Test loss={loss:.4f}")
...
if __name__ == "__main__":
  ...
  # Final evaluation with best config on test set
    #print("\n" + "="*60)
    #print("FINAL — Best config retrained, evaluated on test set")
    #print("="*60)
    #sizes = [INPUT_SIZE] + [256] + [OUTPUT_SIZE]
    #net = Network(sizes, optimizer="adam", lambda_=0.0)
    #net.train(train_data, train_class, val_data, val_class,
    #          epochs=20, mini_batch_size=128, eta=0.001)
    #acc, loss = evaluate(net, test_data, test_class)
    #print(f"  Test acc={acc:.4f}  Test loss={loss:.4f}")
